...
def build_game(kind, league, home, away, logo_dir=None):
    ...
    game = {
        ...
        # Deliberately no logo paths. The first version of this guard loaded
        # real logos from the core checkout and 21 of its 72 goldens differed
        # in CI on the *same* Pillow -- because the logo files themselves vary
        # between checkouts (main dropped case-colliding duplicates), so the
        # goldens encoded one machine's asset set rather than the plugin's
        # rendering. Without them the only inputs are the repo's own fonts.
        # The geometry is still exercised: _logo_slot_width and
        # _center_gap_width are computed to lay the card out either way.
        "home_logo_path": None,
        "away_logo_path": None,
...
def main(update: bool) -> int:
    ...
    # The renderers log an error per missing logo. That is expected here -- the
    # cards are deliberately logo-free -- and 144 such lines would bury a real
            ...
            for kind in TYPES:
                ...
                try:
                    ...
                    img = renderer.render_game_card(
                        build_game(kind, league, home, away), kind
                    ).convert("RGB")
                except Exception as exc:              # noqa: BLE001
