...
Run: <core-venv>/bin/python plugins/lacrosse-scoreboard/test_lookahead_window_is_enforced.py
"""
...
def _make(events, favorites, lookahead=7):
    ...
    obj.league = "ncaa_mens_lacrosse"
    obj.sport = "lacrosse"
    obj.sport_key = "ncaa_mens_lacrosse"
...
def main():
    events = [
        _upcoming("near", "SYR", "UVA", days_out=3),
        _upcoming("edge", "DUKE", "UNC", days_out=6),
        _upcoming("far", "JHU", "MD", days_out=27),        # favourite, but weeks out
        _upcoming("mid", "ND", "PSU", days_out=13),
    ]

    print("a 7-day window keeps the near games and drops the distant ones")
    probe = _make(events, favorites=["MD"])
    probe.update()
    ...
    probe = _make(events, favorites=["MD"], lookahead=30)
    probe.update()
    shown = sorted(g["id"] for g in probe.games_list)
