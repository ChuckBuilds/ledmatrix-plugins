#!/usr/bin/env python3
"""
Tests for lacrosse_timezone.resolve_timezone_name -- the resolution order that
...
Run: <core-venv>/bin/python plugins/lacrosse-scoreboard/test_timezone_resolution.py
"""
...
import lacrosse_timezone  # noqa: E402
from lacrosse_timezone import resolve_timezone, resolve_timezone_name  # noqa: E402
...
_real_system_timezone_name = lacrosse_timezone.system_timezone_name
...
def test_lower_priority_sources_are_not_evaluated():
    ...
    lacrosse_timezone.system_timezone_name = lambda: system_calls.append(1) or None
    try:
        name = resolve_timezone_name(
        ...
        lacrosse_timezone.system_timezone_name = _real_system_timezone_name
...
def test_system_timezone_backstop():
    lacrosse_timezone.system_timezone_name = lambda: "America/Chicago"
    try:
...
def test_utc_last_resort():
    lacrosse_timezone.system_timezone_name = lambda: None
    try:
...
def test_user_set_utc_is_never_overridden():
    """This plugin never wrote back to the saved config, so a plugin-level
    "UTC" can only be the user's own choice and must be honored."""
    name = resolve_timezone_name(
        config={"timezone": "UTC"},
    ...
    assert name == "UTC", name
    print("✓ a user-set 'UTC' is honored (no write-back bug in this plugin)")
...
def test_utc_is_kept_when_nothing_disagrees():
    ...
    lacrosse_timezone.system_timezone_name = lambda: "UTC"
    try:
        name = resolve_timezone_name(
            config={"timezone": "UTC"},
...
def test_present_global_key_still_wins_over_system_zone():
    lacrosse_timezone.system_timezone_name = lambda: "America/Denver"
    try:
        name = resolve_timezone_name(
            config={},
            plugin_manager=_Holder(_RealCoreConfigManager({"timezone": "America/Chicago"})),
...
def main():
    tests = [
        ...
        test_user_set_utc_is_never_overridden,
        test_user_set_utc_beats_the_system_zone,
