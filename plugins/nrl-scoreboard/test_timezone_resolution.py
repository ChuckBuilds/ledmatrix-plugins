#!/usr/bin/env python3
"""
Tests for nrl_timezone.resolve_timezone_name -- the resolution order that
...
Run: <core-venv>/bin/python plugins/nrl-scoreboard/test_timezone_resolution.py
"""
...
import nrl_timezone  # noqa: E402
from nrl_timezone import resolve_timezone, resolve_timezone_name  # noqa: E402
...
_real_system_timezone_name = nrl_timezone.system_timezone_name
...
def test_lower_priority_sources_are_not_evaluated():
    ...
    nrl_timezone.system_timezone_name = lambda: system_calls.append(1) or None
    try:
        name = resolve_timezone_name(
        ...
        nrl_timezone.system_timezone_name = _real_system_timezone_name
...
def test_system_timezone_backstop():
    nrl_timezone.system_timezone_name = lambda: "America/Chicago"
    try:
...
def test_utc_last_resort():
    nrl_timezone.system_timezone_name = lambda: None
    try:
...
def test_utc_is_kept_when_nothing_disagrees():
    ...
    nrl_timezone.system_timezone_name = lambda: "UTC"
    try:
        name = resolve_timezone_name(
            config={"timezone": "UTC"},
...
def test_present_global_key_still_wins_over_system_zone():
    nrl_timezone.system_timezone_name = lambda: "America/Denver"
    try:
        name = resolve_timezone_name(
            config={},
            plugin_manager=_Holder(_RealCoreConfigManager({"timezone": "America/Chicago"})),
