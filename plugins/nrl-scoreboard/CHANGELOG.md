...
### Changed
- **6 config control(s) that cannot affect anything are no longer drawn.**
...
- **The rankings fetch ran on leagues that publish no poll.**
  ...
  `show_ranking` ones, so ticking Show Ranking on the NRL sent two requests an
...
- **Scores and schedules load again after ESPN stopped accepting date
  ...
  NRL boards logged `400 Client Error` and showed nothing. A rejected
  range is now fetched by `fetch_espn_scoreboard` (`nrl_espn_dates.py`, a
