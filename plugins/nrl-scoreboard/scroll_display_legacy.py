...
Scroll Display Handler for NRL Scoreboard Plugin
...
class LegacyScrollDisplay:
    """
    Handles scroll mode display for the NRL Scoreboard plugin.
    ...
    def _configure_scroll_helper(self) -> None:
        ...
        self.logger.info(
            f"[NRL Scroll] ScrollHelper configured: {pixels_per_frame:.2f} px/frame, "
    ...
    def display_scroll_frame(self) -> bool:
        ...
        try:
            ...
            if self._frame_count % 300 == 0:  # Log every ~10 seconds at 30fps
                ...
                self.logger.debug(
                    f"[NRL Scroll] Frame {self._frame_count}, "
...
class LegacyScrollDisplayManager:
    ...
    This class provides a higher-level interface for the NRL plugin
