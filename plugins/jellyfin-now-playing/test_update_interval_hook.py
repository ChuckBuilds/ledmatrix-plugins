...
This plugin's manifest says 10, and it had no hook, so a poll interval under 10 seconds did nothing.
...
Run: LEDMATRIX_CORE=/path/to/LEDMatrix python plugins/jellyfin-now-playing/test_update_interval_hook.py
...
PLUGIN_ID = "jellyfin-now-playing"
failures = []
...
plugin = JellyfinNowPlayingPlugin(
    PLUGIN_ID,
...
print("configured update_interval 5 (manifest %s)" % MANIFEST_INTERVAL)
got = scheduler_interval(plugin)
check("scheduler uses the configured interval, not the manifest's",
      got == 5, "got %r" % got)

print("changed to 30 from the web UI")
plugin.on_config_change({"enabled": True, "update_interval": 30})
got = scheduler_interval(plugin)
