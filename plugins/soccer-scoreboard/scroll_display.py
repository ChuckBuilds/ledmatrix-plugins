"""
Scroll Display Handler for Soccer Scoreboard.
...
class ScrollDisplay(_ScrollDisplayBase):
    """Soccer game cards and separator icons on the core scroll engine."""

    # This lineage reads one `scroll_mode` block rather than walking
    # per-league keys, so there is no league ladder.
    SCROLL_LEAGUE_KEYS = ()
    ...
    def scroll_settings_defaults(self):
        ...
        # game cards pinned at 128px where core sizes them to the panel.
        # Core's own max_duration default is 600; this lineage uses 300.
        return {
    ...
    def __init__(self, *args, **kwargs):
        # The legacy constructor took plugin_dir as a positional argument;
        # the core base does not. It was only ever the directory this
        # module lives in -- the legacy manager computed exactly this --
        # so derive it and keep core's signature, which lets core's
        # get_scroll_display() construct us unchanged.
        #
        # Set before super(): the base calls _load_separator_icons() from
        # its __init__, and that reads self.plugin_dir.
...
class ScrollDisplayManager(_ScrollDisplayManagerBase):
    """Soccer scroll manager -- everything but the extras below is core."""
    ...
    def prepare_content(self, games, game_type, leagues, rankings_cache=None) -> bool:
        ...
        the active display, so the next display_frame() would scroll the Vegas
        slate instead of the mode actually on screen. Ported from baseball.
        """
    ...
    def get_vegas_content_items_for(self, game_type: str) -> list:
        """A copy of one display's Vegas items, or [] if it has none.

        Vegas needs the combined 'mixed' display specifically, not the union
        of every display the standalone modes happen to have rendered.
        """
