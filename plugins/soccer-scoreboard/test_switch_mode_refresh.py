...
class _Stub(_Base):
    ...
    def __init__(self, leagues=("eng.1", "esp.1"), missing=()):
        ...
        # The real registry hands back the same manager every time; the
        # dispatch keys its bookkeeping on that identity.
        self._managers = {key: _Manager(key) for key in self._leagues}
    ...
    def _get_league_manager_for_mode(self, league_key, mode_type):
        if league_key in self._missing:
            return None
        return self._managers[league_key]
