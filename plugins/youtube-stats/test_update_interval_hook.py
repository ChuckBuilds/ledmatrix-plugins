...
This plugin's manifest says 300, and it had no hook, so an interval under 300 seconds did nothing, despite the 60-second floor the README promises.
...
Run: LEDMATRIX_CORE=/path/to/LEDMatrix python plugins/youtube-stats/test_update_interval_hook.py
...
plugin = YouTubeStatsPlugin(
    PLUGIN_ID,
    {"enabled": True, "update_interval": 60},
    VisualTestDisplayManager(128, 32),
    MockCacheManager(),
    MockPluginManager(),
)

print("configured update_interval 60 (manifest %s)" % MANIFEST_INTERVAL)
got = scheduler_interval(plugin)
check("scheduler uses the configured interval, not the manifest's",
      got == 60, "got %r" % got)

print("changed to 900 from the web UI")
plugin.on_config_change({"enabled": True, "update_interval": 900})
got = scheduler_interval(plugin)
