...
that the baseball end-of-game rules themselves are unchanged.

Run: <core-venv>/bin/python plugins/baseball-scoreboard/test_null_period_really_over.py
...
check("live game is not over", shared_is_over,
      {"period_text": "Top 2nd", "period": 2, "clock": "5:00"}, False)
...
check("0:00 in period 4 is over", shared_is_over,
      {"period_text": "Bot 9th", "period": 4, "clock": "0:00"}, True)
check("0:00 in period 1 is not over", shared_is_over,
      {"period_text": "Top 2nd", "period": 1, "clock": "0:00"}, False)
...
check("period None is period 0 (not over at 0:00)", shared_is_over,
      {"period_text": "Top 2nd", "period": None, "clock": "0:00"}, False)
check("non-numeric period string is period 0", shared_is_over,
      {"period_text": "Top 2nd", "period": "OT", "clock": "0:00"}, False)
check("period_text and period both None", shared_is_over,
      {"period_text": None, "period": None, "clock": "5:00"}, False)
check("no clock and a None period is not over", shared_is_over,
      {"period_text": None, "period": None, "clock": None}, False)
...
# BaseballLive adds a not-played-status check, then defers to the shared one.
_live = object.__new__(_ConcreteLive)
_live.logger = logging.getLogger("null_period_probe")

print("\nBaseballLive._is_game_really_over")
check("in-progress game with null period and period_text", _live._is_game_really_over,
      {"status": "status_in_progress", "period_text": None, "period": None}, False)
check("postponed game is still over", _live._is_game_really_over,
      {"status": "status_postponed", "period_text": None, "period": None}, True)
check("None status with a 'Final' period_text", _live._is_game_really_over,
      {"status": None, "period_text": "Final", "period": "junk"}, True)
