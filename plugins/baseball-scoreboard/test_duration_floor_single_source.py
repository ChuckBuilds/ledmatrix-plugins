...
Run: <core-venv>/bin/python plugins/baseball-scoreboard/test_duration_floor_single_source.py
"""
...
def make_plugin(league_blocks):
    ...
    p = BaseballScoreboardPlugin("baseball-scoreboard", config, dm, MagicMock(), MagicMock())
...
CASES = [
    ("no floor configured", {"mlb": {"enabled": True}}, "mlb_recent", None),
    ("per-league floor", {"mlb": {"enabled": True, "dynamic_duration": dyn(60)}},
     "mlb_recent", 60.0),
    ("per-mode floor beats per-league",
     {"mlb": {"enabled": True, "dynamic_duration": dyn(60, {"recent": 90})}},
     "mlb_recent", 90.0),
    ("per-mode floor for another mode falls back to per-league",
     {"mlb": {"enabled": True, "dynamic_duration": dyn(60, {"live": 90})}},
     "mlb_recent", 60.0),
    ("highest floor across enabled leagues",
     {"mlb": {"enabled": True, "dynamic_duration": dyn(40)},
      "ncaa_baseball": {"enabled": True, "dynamic_duration": dyn(75)}},
     "mlb_recent", 75.0),
    ("disabled league's floor is ignored",
     {"mlb": {"enabled": True, "dynamic_duration": dyn(40)},
      "milb": {"enabled": False, "dynamic_duration": dyn(500)}},
     "mlb_recent", 40.0),
    ("zero and junk floors are ignored",
     {"mlb": {"enabled": True, "dynamic_duration": dyn(0, {"recent": "abc"})}},
     "mlb_recent", None),
    ("combined upcoming mode, milb only",
     {"milb": {"enabled": True, "dynamic_duration": dyn(None, {"upcoming": 55})}},
     "baseball_upcoming", 55.0),
]


def main():
    check("the unused get_dynamic_duration_floor copy is gone",
          not hasattr(BaseballScoreboardPlugin, "get_dynamic_duration_floor"))
    ...
    for label, blocks, display_mode, expected in CASES:
        ...
        games = sum(1 for lg in LEAGUES if blocks.get(lg, {}).get("enabled")) \
            if display_mode.startswith("baseball_") else 1
