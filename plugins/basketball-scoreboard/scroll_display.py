...
class ScrollDisplayManager(_ScrollDisplayManagerBase):
    ...
    def prepare_content(
        ...
        ``_current_game_type`` and the next display_frame() would draw the
        Vegas slate instead.
        """
    ...
    def get_vegas_content_items_for(self, game_type: str) -> list:
        """
        The Vegas items of a single scroll display, not the union of all.

        get_all_vegas_content_items() returns whatever the standalone modes
        last rendered: only that mode's games, and a game twice when two
        displays hold it.
        """
