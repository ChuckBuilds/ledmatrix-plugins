...
background cache, so every fixture ESPN had published was eligible: so
Upcoming could fill with games weeks beyond the configured horizon.
SportsUpcoming.update() now applies the cutoff to selection, mirroring
the lookback cutoff on the Recent screen (ported from football-scoreboard
#345).
...
Run: <core-venv>/bin/python plugins/basketball-scoreboard/test_lookahead_window_is_enforced.py
"""
...
def main():
    events = [
        _upcoming("near", "BOS", "SEA", days_out=3),
        _upcoming("edge", "CLE", "CHI", days_out=6),
        _upcoming("far", "MIN", "TOR", days_out=27),        # favourite, but weeks out
        _upcoming("mid", "LV", "LAL", days_out=13),
    ]

    print("a 7-day window keeps the near games and drops the distant ones")
    probe = _make(events, favorites=["TOR"])
    probe.update()
    ...
    probe = _make(events, favorites=["TOR"], lookahead=30)
    probe.update()
    shown = sorted(g["id"] for g in probe.games_list)
