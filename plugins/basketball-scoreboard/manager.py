...
Features:
- Multiple league support (NBA, NCAA M/W, WNBA)
...
# Import basketball base classes from LEDMatrix
try:
...
class BasketballScoreboardPlugin(BasePlugin):
    """
    Basketball scoreboard plugin for displaying games across multiple leagues.

    Supports NBA, NCAA Men's Basketball, NCAA Women's Basketball, and WNBA
    with live, recent, and upcoming game modes.

    Configuration options:
        leagues: Enable/disable NBA, NCAA M, NCAA W, WNBA
    ...
    ESPN_API_URLS = {
        ...
        'ncaam_basketball': 'https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/scoreboard',
        'ncaaw_basketball': 'https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/scoreboard',
    ...
    def __init__(self, plugin_id: str, config: Dict[str, Any],
        ...
        if Basketball is None:
            self.logger.error("Failed to import Basketball base classes. Plugin will not function.")
        ...
        self.logger.info("Basketball scoreboard plugin initialized")
    ...
    def _register_fonts(self):
        ...
        try:
            ...
            self.logger.info("Basketball scoreboard fonts registered")
        except Exception as e:
            self.logger.warning(f"Error registering fonts: {e}")

    def update(self) -> None:
        """Update basketball game data for all enabled leagues."""
        ...
        try:
            ...
            self.logger.debug(f"Updated basketball data: {len(self.current_games)} games")

        except Exception as e:
            self.logger.error(f"Error updating basketball data: {e}")
    ...
    def display(self, display_mode: str = None, force_clear: bool = False) -> None:
        ...
        Args:
            display_mode: Which mode to display (basketball_live, basketball_recent, basketball_upcoming)
            force_clear: If True, clear display before rendering
        """
        if not self.initialized:
            self._display_error("Basketball plugin not initialized")
            return
        ...
        if not display_mode:
            ...
            if self._has_live_games():
                display_mode = 'basketball_live'
            else:
                # Fall back to recent or upcoming
                display_mode = 'basketball_recent' if self._has_recent_games() else 'basketball_upcoming'
    ...
    def _filter_games_by_mode(self, mode: str) -> List[Dict]:
        ...
        for game in self.current_games:
            ...
            mode_enabled = display_modes.get(mode.replace('basketball_', ''), False)
            if not mode_enabled:
                continue

            # Filter by game state and per-league limits
            if mode == 'basketball_live' and state == 'in':
                filtered.append(game)

            elif mode == 'basketball_recent' and state == 'post':
            ...
            elif mode == 'basketball_upcoming' and state == 'pre':
    ...
    def _display_no_games(self, mode: str):
        ...
        message = {
            'basketball_live': "No Live Games",
            'basketball_recent': "No Recent Games",
            'basketball_upcoming': "No Upcoming Games"
        }.get(mode, "No Games")
    ...
    def cleanup(self) -> None:
        """Cleanup resources."""
        self.current_games = []
        self.logger.info("Basketball scoreboard plugin cleaned up")
