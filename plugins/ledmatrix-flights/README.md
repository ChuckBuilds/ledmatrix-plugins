...
| `live_priority` | `false` | When enabled, an aircraft entering the proximity radius immediately preempts the normal rotation to show the overhead view for the proximity alert window (see proximity_alert.duration_seconds). The 'flight_tracker_live' rotation slot is always registered and is skipped unless this is on and a plane is overhead, so switching it on takes effect without a restart. Leave off for legacy behavior. Advanced. |
