...
The copy also compared ``game.get("period", 0) >= 3``, which raised
...
that the hockey end-of-game rules themselves are unchanged.

Run: <core-venv>/bin/python plugins/hockey-scoreboard/test_null_period_really_over.py
...
check("live game is not over", shared_is_over,
      {"period_text": "P2", "period": 2, "clock": "5:00"}, False)
check("'Final' in period_text is over", shared_is_over,
      {"period_text": "Final", "period": 3, "clock": "0:00"}, True)
check("0:00 in period 3 is over", shared_is_over,
      {"period_text": "P3", "period": 3, "clock": "0:00"}, True)
check("0:00 in period 1 is not over", shared_is_over,
      {"period_text": "P2", "period": 1, "clock": "0:00"}, False)
...
check("period_text None still reaches the 0:00 check", shared_is_over,
      {"period_text": None, "period": 3, "clock": "0:00"}, True)
check("period None is period 0 (not over at 0:00)", shared_is_over,
      {"period_text": "P2", "period": None, "clock": "0:00"}, False)
check("non-numeric period string is period 0", shared_is_over,
      {"period_text": "P2", "period": "OT", "clock": "0:00"}, False)
