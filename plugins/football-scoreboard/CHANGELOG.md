...
### Changed
- **5 config control(s) that cannot affect anything are no longer drawn.**
  ...
  - `other_games_divisions` (×1) — the FBS / FCS / Other checkboxes. ESPN
  ...
  Left visible where the league genuinely publishes a poll: `ncaa_fb`.
...
- **The rankings fetch ran on leagues that publish no poll.**
  ...
  `show_ranking` ones, so ticking Show Ranking on the NFL sent two requests an
