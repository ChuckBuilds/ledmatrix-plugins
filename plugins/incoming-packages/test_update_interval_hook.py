...
This plugin's manifest says 600, and it had no hook, so a refresh interval under 600 seconds did nothing.
...
Run: LEDMATRIX_CORE=/path/to/LEDMatrix python plugins/incoming-packages/test_update_interval_hook.py
...
plugin = IncomingPackagesPlugin(
    PLUGIN_ID,
    {"enabled": True, "update_interval": 120},
    VisualTestDisplayManager(128, 32),
    MockCacheManager(),
    MockPluginManager(),
)

print("configured update_interval 120 (manifest %s)" % MANIFEST_INTERVAL)
got = scheduler_interval(plugin)
check("scheduler uses the configured interval, not the manifest's",
      got == 120, "got %r" % got)

print("changed to 1800 from the web UI")
plugin.on_config_change({"enabled": True, "update_interval": 1800})
got = scheduler_interval(plugin)
