"""
Scroll Display -- LEGACY FALLBACK.

Used only on a core that predates `src.common.sports_scroll` (LEDMatrix
3.2.0). `scroll_display.py` prefers the core implementation and falls back
...
Scroll Display Handler for AFL Scoreboard Plugin
...
class LegacyScrollDisplay:
    """
    Handles scroll mode display for the AFL Scoreboard plugin.
    ...
    def _configure_scroll_helper(self) -> None:
        ...
        self.logger.info(
            f"[AFL Scroll] ScrollHelper configured: {pixels_per_frame:.2f} px/frame, "
    ...
    def display_scroll_frame(self) -> bool:
        ...
        try:
            ...
            if self._frame_count % 300 == 0:  # Log every ~10 seconds at 30fps
                ...
                self.logger.debug(
                    f"[AFL Scroll] Frame {self._frame_count}, "
...
class LegacyScrollDisplayManager:
    ...
    This class provides a higher-level interface for the afl plugin
    ...
    def get_scroll_display(self, game_type: str) -> 'LegacyScrollDisplay':
        ...
        if game_type not in self._scroll_displays:
            ...
            self._scroll_displays[game_type] = LegacyScrollDisplay(
