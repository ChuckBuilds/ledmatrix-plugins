...
This sport's clock counts up, so the check only looks for "final" in
the period text; the period is only logged, and must stay loggable.
...
that the AFL end-of-game rules themselves are unchanged.

Run: <core-venv>/bin/python plugins/afl-scoreboard/test_null_period_really_over.py
...
# Unchanged behaviour: only "final" in the period text ends a game.
check("live game is not over", shared_is_over,
      {"period_text": "Q3 12:34", "period": 2, "clock": "67'"}, False)
check("'Final' in period_text is over", shared_is_over,
      {"period_text": "Final", "period": 2, "clock": ""}, True)

# Null / junk values.
check("period_text None", shared_is_over,
      {"period_text": None, "period": 2, "clock": "67'"}, False)
check("period None", shared_is_over,
      {"period_text": "Q3 12:34", "period": None, "clock": "67'"}, False)
check("non-numeric period string", shared_is_over,
      {"period_text": "Q3 12:34", "period": "ET", "clock": "67'"}, False)
check("period_text and period both None", shared_is_over,
      {"period_text": None, "period": None, "clock": ""}, False)
check("'Final' still wins with a None period", shared_is_over,
      {"period_text": "Final", "period": None, "clock": ""}, True)
