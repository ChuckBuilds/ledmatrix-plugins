...
from afl_timezone import resolve_timezone
...
def _clamp_window(value: Any, fallback: int) -> int:
    ...
    try:
        ...
        # from SportsCore.__init__, where it would fail manager construction.
        return fallback
...
# ESPN statuses that end a fixture without it being played out. ESPN files
# these under state "post" -- the same bucket as a finished game -- with both
# scores "0", so keying Recent eligibility on the state alone drew a washed-out
# or called-off match as "Final 0-0".
_UNPLAYED_STATUS_NAMES = frozenset({
    "STATUS_POSTPONED",
    "STATUS_CANCELED",
    "STATUS_CANCELLED",
    "STATUS_SUSPENDED",
...
def _status_is_final(status_type: Any) -> bool:
    """True only for a game that was actually played to a result.

    Requires state "post", a completed flag that is not False, and a status
    name outside _UNPLAYED_STATUS_NAMES. A missing completed flag is treated
    as completed so a payload that omits it keeps its old behaviour; ESPN
    itself always sends it, and sends False for postponements.
    """
    if not isinstance(status_type, dict) or status_type.get("state") != "post":
        return False
    if status_type.get("completed", True) is False:
        return False
    return str(status_type.get("name") or "").upper() not in _UNPLAYED_STATUS_NAMES
...
class SportsCore(SportsCoreSharedMixin, ABC):
    ...
    def __init__(
        ...
        # Use LogoDownloader to get the correct default logo directory for AFL
        default_logo_dir = Path(LogoDownloader().get_logo_directory("afl"))
        ...
        # Bounded LRU of decoded logos (core #559): a plain dict grew with
        # every abbreviation ever drawn for the life of the process.
    ...
    def _get_logo_directory_fallbacks(self, configured_dir: Path) -> List[Path]:
        ...
        if env_override:
            ...
            fallbacks.append(env_path / "afl_logos")

        cache_dir = getattr(self.cache_manager, "cache_dir", None)
        if cache_dir:
            fallbacks.append(Path(cache_dir) / "logos" / "afl")

        try:
            fallbacks.append(Path.home() / ".ledmatrix" / "logos" / "afl")
        except Exception as e:
            self.logger.debug(f"Could not resolve home directory for logo fallback: {e}")

        fallbacks.append(Path(tempfile.gettempdir()) / "ledmatrix_logos" / "afl")
    ...
    def _upcoming_date_and_time_text(self, game_date: str, game_time: str,
        ...
        Ported from football-scoreboard (#342).
        """
    ...
    def _odds_would_hit_top_row(span, placements) -> bool:
        ...
        Decided by measurement rather than panel size, same rule as
        game_renderer: a two-digit total can collide where a one-digit one
        clears. One pixel of breathing room either side.
        """
        if not span:
            return False
        left, right = span
    ...
    def _draw_dynamic_odds(
        ...
        ``top_span`` is the (left, right) extent of the text the card centres
        on the top row (period/clock, "Final", the league header), so the
        odds can step down a row instead of printing through it.
        """
        try:
            ...
            # is truly missing: a home spread of 0.0 is a real pick'em line,
            # and the top-level value is not guaranteed to be a number, so it
            # is only negated when it is one (matches game_renderer).
            ...
            # Work out every label and its span before drawing any, so the
            # row is chosen once with full knowledge of what has to fit.
            placements = []
            if favored_spread is not None:
                # The favoured side's spread goes on that side's edge.
            ...
            # Over/under on the opposite edge from the spread.
            ...
            if over_under is not None and isinstance(over_under, (int, float)):
                ...
                if favored_side == "away":
                    ...
                    # case put it straight through the period/"Final"/header
                    # text the card centres on this same row; anchor it to the
                    # left edge like the scroll card does (game_renderer).
                    ou_x = 0
            ...
            if self._odds_would_hit_top_row(top_span, placements):
                # Step down one text row rather than print through it.
    ...
    #: still bounding the cache (ported from core #559).
    ...
    def _load_and_resize_logo(
        ...
        try:
            ...
            if not actual_logo_path:
                ...
                # LogoDownloader keys the cache directory by this identifier.
                league = "afl"
    ...
    def _attach_odds_to_rotated_games(self, games: List[Dict]) -> None:
        ...
        odds update() gave them. Ported from football-scoreboard (#343).
        ...
        """
        # getattr: the rotation is reached from probes and partially built
        # managers that never set show_odds; no attribute means no odds.
    ...
    def _reset_dwell_on_reentry(self) -> bool:
        ...
        defect: the dwell clock counting time the viewer never saw. Ported
...
class SportsUpcoming(SportsCore):
    ...
    def update(self):
        ...
        try:
            ...
            # this horizon, but the shared scoreboard cache can hold a wider
            # window (another manager, or a larger lookahead before a config
            # change), so selection applies the cutoff too. Mirrors the
            # lookback cutoff on the Recent screen. Ported from
...
class SportsLive(SportsLiveSharedMixin, SportsCore):
    ...
    def _get_live_status_text(self, game: Dict) -> str:
        ...
        # afl managers; fall back to the raw clock, then a generic "LIVE".
    ...
    def _draw_scorebug_layout(self, game: Dict, force_clear: bool = False) -> None:
        """Draw the switch-mode scorebug for a live afl game.
    ...
    def _is_game_really_over(self, game: Dict) -> bool:
        ...
        AFL-specific: clock counts UP (e.g., 75', 90+3'), so we check for
