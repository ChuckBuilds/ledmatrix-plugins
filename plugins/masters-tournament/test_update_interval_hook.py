...
This plugin's manifest says 30, and it had no hook, so an interval under 30 seconds did nothing.
...
Run: LEDMATRIX_CORE=/path/to/LEDMatrix python plugins/masters-tournament/test_update_interval_hook.py
...
from manager import MastersTournamentPlugin  # noqa: E402

PLUGIN_ID = "masters-tournament"
failures = []
...
plugin = MastersTournamentPlugin(
    PLUGIN_ID,
    {"enabled": True, "update_interval": 15},
    VisualTestDisplayManager(128, 32),
    MockCacheManager(),
    MockPluginManager(),
)

print("configured update_interval 15 (manifest %s)" % MANIFEST_INTERVAL)
got = scheduler_interval(plugin)
check("scheduler uses the configured interval, not the manifest's",
      got == 15, "got %r" % got)

print("changed to 60 from the web UI")
plugin.on_config_change({"enabled": True, "update_interval": 60})
got = scheduler_interval(plugin)
